...
class Remotefiles extends Model
{
	...
	public function downloadToServer($id, $part, $frag)
	{
		...
		if ($part == -1)
		{
			...
			$this->container->segment->set('dl_totalsize', $backupRecord['total_size']);
			// Cumulative bytes downloaded so far
			$this->container->segment->set('dl_donesize', 0);
		...
		while (true)
		{
			...
			$currentTotal           = $this->container->segment->get('dl_donesize', 0);
			$this->container->segment->set('dl_donesize', $currentTotal + $downloadedFragmentSize);
	...
	private function getPostProcEngineNameForRecord($id, $profileOverridesRecord = false)
	{
		...
		{
			/** @var Manage $profilesModel */
			$profilesModel      = $this->container->mvcFactory->makeTempModel('Manage');
