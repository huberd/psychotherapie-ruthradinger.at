...
class Restore extends Model
{
	...
	public function getLatestBackupForProfile($profileID)
	{
		...
		$transferModel = $this->container->mvcFactory->makeTempModel('Transfers');
...
ENDDATA;
	...
	public function getExtractionModes()
	{
		...
		$options[] = $this->getContainer()->html->select->option( 'hybrid',	Text::_('COM_AKEEBA_RESTORE_LABEL_EXTRACTIONMETHOD_HYBRID'));
		$options[] = $this->getContainer()->html->select->option( 'direct',	Text::_('COM_AKEEBA_RESTORE_LABEL_EXTRACTIONMETHOD_DIRECT'));
		$options[] = $this->getContainer()->html->select->option( 'ftp',		Text::_('COM_AKEEBA_RESTORE_LABEL_EXTRACTIONMETHOD_FTP'));
