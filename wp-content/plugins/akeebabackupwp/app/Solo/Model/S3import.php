...
class S3import extends Model
{
	...
	public function getBucketsDropdown()
	{
		...
		$options[] = $this->getContainer()->html->select->option('', Text::_('COM_AKEEBA_S3IMPORT_LABEL_SELECTBUCKET'));

		if (!empty($buckets))
		{
			foreach ($buckets as $b)
			{
				$options[] = $this->getContainer()->html->select->option($b, $b);
			}
		}
		...
		return $this->getContainer()->html->select->genericList(
			$options, 's3bucket', ['class' => 'form-control'], 'value', 'text', $selected
		);
	}
	...
	public function downloadToServer()
	{
		...
		while (($timer->getRunningTime() < $this->maxTimeAllowance) && !$break && ($part < $totalparts))
		{
			...
			if (!$result)
			{
				...
				if (
					(
						(($part < $totalparts) || (($totalparts == 1) && ($part == 0))) && ($frag == 0)
					)
				)
				{
