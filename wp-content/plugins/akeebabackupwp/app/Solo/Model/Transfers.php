...
class Transfers extends Model
{
	...
	public function getLatestBackupInformation($profileID = null)
	{
		...
		/** @var Manage $model */
		$model = $this->container->mvcFactory->makeTempModel('Manage');
		$model->savestate(0);
	...
	{
		$randomPrefix = strtolower((new RandomValue())->generateString(16));
	...
	public function initialiseUpload(array $config)
	{
		/** @var Transfer\TransferInterface $connector */
		$connector = $this->getConnector($config);
		$randomPrefix = $this->container->segment->get('transfer.randomName', 'kickstart');

		// Can I upload Kickstart and my extra script?
		$files = array(
			APATH_ROOT . '/Solo/assets/installers/kickstart.txt'
			=> $randomPrefix . '.php',
			APATH_ROOT . '/Solo/assets/installers/kickstart.transfer.php'
