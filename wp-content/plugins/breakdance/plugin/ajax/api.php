            ...
            if (!$allowed) {
                if (!is_user_logged_in()) {
                    throw new BreakdanceAjaxHandlerException(
                        "You need to be logged in and have minimum permission level of \"{$minimum_permissions}\"",
                        401
                    );
                }
...
JS;
        ...
        if ($hasRefreshNonceQueryParam) {
            add_action('admin_notices', static function () {
                echo '<div class="updated"><p>Your ' . __bdox('plugin_name') . ' session token has been refreshed</p></div>';
            });
        }
    }
}
