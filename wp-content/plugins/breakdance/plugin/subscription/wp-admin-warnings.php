  ...
  if (isFreeMode()) {
    add_action('admin_notices', function () {
      if (filter_input(INPUT_GET, 'breakdance_form_submissions_upgrade_pro', FILTER_UNSAFE_RAW)) {
        showWpAdminUpgradeToProNotice("Please upgrade to " . __bdox('plugin_name') . " Pro to export form submissions.", 'forms');
      }
    });
    ...
    add_action('admin_notices', function () {
      ...
      if ($isBreakdanceSettingsPage){
        ...
        if ($currentTab === 'design_library') {
          showWpAdminUpgradeToProNotice("When the design library comes out of beta, it will only be available in " . __bdox('plugin_name') . " Pro.", 'design-library');
        }
  
        else if ($currentTab === 'permissions') {
          showWpAdminUpgradeToProNotice("Please upgrade to " . __bdox('plugin_name') . " Pro to configure permissions.", 'user-access');
        }
  
        else if ($currentTab === 'tools') {
          showWpAdminUpgradeToProNotice("Please upgrade to " . __bdox('plugin_name') . " Pro to import and export settings.", 'import-export');
        }
      }
    });
  }
...
.breakdance-wp-admin-upgrade-to-pro-notice {
  ...
  <div class='breakdance-wp-admin-upgrade-to-pro-notice'>
    ...
    ?>
    <?php echo $text; ?>
    <a href='https://<?php echo $domain; ?>/?utm_source=free-version&utm_medium=free-version&utm_campaign=<?php echo $feature; ?>' target='_blank' class='breakdance-wp-admin-upgrade-to-pro-notice-button'>Get Pro</a>
  </div>
  <?php
}
