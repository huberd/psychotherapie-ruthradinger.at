...
( function ( O, pa ) {
			...
			if ( void 0 === c || e )
				e
					...
					: ( document.body.addEventListener( b, d, ! 0 ),
					  a.on( 'destroy', () =>
							document.body.removeEventListener( b, d, ! 0 )
					  ) );
			else
	...
	function U( a ) {
		return ( ( fa( a ) ? a.ownerDocument : a.document ) || window.document )
			.documentElement;
	}
	function wa( a ) {
		return 'html' === M( a )
		...
		if ( 'viewport' === b ) {
			...
			fa( b )
				...
				: ( ( f = U( a ) ),
				  ( a = U( f ) ),
				  ( d = Ha( f ) ),
				  ( b = null == ( c = f.ownerDocument ) ? void 0 : c.body ),
				  ( c = L(
						a.scrollWidth,
						a.clientWidth,
						b ? b.scrollWidth : 0,
				  ...
				  ( d = -d.scrollTop ),
				  'rtl' === P( b || a ).direction &&
						( f += L( a.clientWidth, b ? b.clientWidth : 0 ) - c ),
				  ( a = Ka( { width: c, height: e, x: f, y: d } ) ) );
		return a;
	}
	function Lb( a ) {
		var b = sa( wa( a ) ),
			c =
				0 <= [ 'absolute', 'fixed' ].indexOf( P( a ).position ) &&
				F( a )
					? ra( a )
					: a;
		return fa( c )
	...
	function ta( a, b ) {
		...
		e = Mb(
			fa( l ) ? l : l.contextElement || U( a.elements.popper ),
			e,
			f
		);
		...
		return {
			c() {
				...
				B(
					b,
					'aria-label',
					( d = a[ 0 ].label ? a[ 0 ].label : 'Close Tour' )
				);
	...
	function jc( a, b, c ) {
		...
		a.$$.update = () => {
			a.$$.dirty & 2 &&
				( c( 2, ( f = e.options.title ) ),
				c( 3, ( g = e.options.cancelIcon ) ) );
		};
		return [ d, e, f, g ];
	}
	function kc( a ) {
			...
			p( q, n ) {
				...
				n & 4 &&
					( c =
						...
						( q[ 2 ].options.cancelIcon &&
							q[ 2 ].options.cancelIcon.enabled ) );
				c
					? m
						? ( m.p( q, n ), n & 4 && z( m, 1 ) )
				...
				n & 4 &&
					( g =
						Array.isArray( q[ 2 ].options.buttons ) &&
						q[ 2 ].options.buttons.length );
				g
					? p
						? ( p.p( q, n ), n & 4 && z( p, 1 ) )
	...
	function rc( a ) {
		...
		let p = [
				...
				{
					'aria-labelledby': ( f = a[ 4 ].options.title
						? a[ 3 ]
						: null ),
				},
				a[ 1 ],
	...
	function tc( a, b, c ) {
		...
		za().$$.after_update.push( () => {
			if ( x !== p.options.classes ) {
				var h = x;
				qa( h ) &&
					( ( h = zb( h ) ), h.length && e.classList.remove( ...h ) );
				h = x = p.options.classes;
				qa( h ) &&
					( ( h = zb( h ) ), h.length && e.classList.add( ...h ) );
			}
		} );
		a.$$set = ( h ) => {
			...
			'firstFocusableElement' in h &&
				c( 8, ( g = h.firstFocusableElement ) );
			'focusableElements' in h && c( 9, ( l = h.focusableElements ) );
			'labelId' in h && c( 3, ( m = h.labelId ) );
			'lastFocusableElement' in h &&
				c( 10, ( k = h.lastFocusableElement ) );
		...
		a.$$.update = () => {
			a.$$.dirty & 16 &&
				( c(
					...
					( n =
						p.options &&
						p.options.cancelIcon &&
						p.options.cancelIcon.enabled )
				),
	...
	function Ab( a ) {
		if ( ! a ) return null;
		let b =
			a instanceof HTMLElement && window.getComputedStyle( a ).overflowY;
		return 'hidden' !== b &&
			'visible' !== b &&
...
Z` )
									...
									for (
										k.rects = {
											reference: Pb(
												x,
												ra( r ),
												'fixed' === k.options.strategy
											),
											popper: Fa( r ),
										},
											k.reset = ! 1,
											k.placement = k.options.placement,
											k.orderedModifiers.forEach(
												function ( v ) {
													return ( k.modifiersData[
														v.name
													] = Object.assign(
														{},
														v.data
			...
			defaultModifiers: [
				{
					...
					effect: function ( a ) {
						...
						return function () {
							...
							f &&
								g.removeEventListener( 'resize', c.update, Da );
						};
					},
					data: {},
				},
				...
				},
				{
					...
					fn: function ( a ) {
						...
						if ( ! b.modifiersData[ a ]._skip ) {
							...
							p = N( c );
							f = f || ( p !== c && q ? Ob( c ) : [ xa( c ) ] );
							var r = [ c ].concat( f ).reduce( function (
								E,
								I
							) {
								return E.concat(
						...
						if ( d ) {
							if ( e ) {
								...
								var na = b.modifiersData[ 'arrow#persistent' ]
									? b.modifiersData[ 'arrow#persistent' ]
											.padding
								...
								q = q
									...
									: p + D + u + h.mainAxis;
								w = ( w =
									b.elements.arrow && ra( b.elements.arrow ) )
									? 'y' === n
										? w.clientTop || 0
										: w.clientLeft || 0
									: 0;
								E =
									null != ( v = null == t ? void 0 : t[ n ] )
										? v
										: 0;
								v = e + q - E;
								y = g ? V( y, e + I - E - w ) : y;
				...
				},
				{
					...
					effect: function ( a ) {
						...
						if ( null != a ) {
							if (
								'string' === typeof a &&
								( ( a = b.elements.popper.querySelector( a ) ),
								! a )
							)
								return;
							Va( b.elements.popper, a ) &&
								( b.elements.arrow = a );
						}
					},
					requires: [ 'popperOffsets' ],
				...
				polyfill: function () {
					...
					function d( h ) {
						if (
							...
							return ! 0;
						if ( 'object' === typeof h && 'smooth' === h.behavior )
							return ! 1;
						throw new TypeError(
					...
					function e( h, t ) {
						if ( 'Y' === t )
							return h.clientHeight + x < h.scrollHeight;
						if ( 'X' === t )
							return h.clientWidth + x < h.scrollWidth;
					}
					function f( h, t ) {
					...
					if (
						...
						q.prototype.scroll = q.prototype.scrollTo = function (
							h,
							t
						) {
							if ( void 0 !== h )
	...
	class Ra extends Qa {
		...
		_setupElements() {
			...
			if ( void 0 !== a && null !== a && a.element && a.on )
				c.placement = a.on;
			else {
				c = Vb();
				var d = {
